...
- **Per-Teacher Trip Cloning**: When a teacher opens a venue review link, they click "Generate My Parent Link" which calls `POST /api/trip/clone` to create a clone trip with a unique `direct_link_token` (format: `t-{sessionPrefix}-{timestamp}`). Each teacher gets their own parent link so responses stay separated. Clone identity uses a `teacher_session_id` stored in localStorage (`tripslip_teacher_session`), and the clone mapping is persisted in `tripslip_cloned_trips`. The clone copies all `trip_forms` from the source trip. Idempotent: same session + same experience/date returns existing clone. Form mutations (add/upload/remove) target the cloned trip when one exists.
