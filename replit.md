...
- **Supabase Authentication**: Each app uses a unique `storageKey` for Supabase client isolation. Authentication flow includes RPC functions for role assignment and teacher creation during signup, bypassing RLS for initial user setup. The teacher portal uses a `/api/signup/link-teacher` proxy endpoint (service role key) to create teacher records since client-side RPC is blocked by RLS. `AuthContext` exposes `teacherLoading` state; `ProtectedRoute` waits for both `loading` and `teacherLoading` before rendering. All protected pages (Dashboard, Trips, Students, Profile) show a shared `NoTeacherProfile` component when no teacher record exists instead of infinite spinners. Trip creation in `ReviewAndSubmitStep` treats missing teacher profile as a hard error (no "demo mode").
...
- **Per-Teacher Trip Cloning**: When a teacher opens a venue review link, they first see a "Welcome, Teacher!" gate requiring their name and school (class/grade optional). This info is saved in localStorage (`tripslip_teacher_info`) and sent with the clone request. They then click "Generate My Parent Link" which calls `POST /api/trip/clone` to create a clone trip with a unique `direct_link_token` (format: `t-{sessionPrefix}-{timestamp}`). Teacher info is stored as JSON in the cloned trip's `special_requirements` field and returned via trip lookup as `teacher_info`. The parent consent form displays the teacher's name, school, and class. Each teacher gets their own parent link so responses stay separated. Clone identity uses a `teacher_session_id` stored in localStorage (`tripslip_teacher_session`), and the clone mapping is persisted in `tripslip_cloned_trips`. The clone copies all `trip_forms` from the source trip. Idempotent: same session + same experience/date returns existing clone. Form mutations (add/upload/remove) target the cloned trip when one exists.
- **Permission Slips Schema**: The `permission_slips` table stores all parent/student data in the `form_data` JSON column and signatures in `signature_data`. There are no `student_name`, `parent_name`, `parent_email`, `parent_phone`, or `signature` columns — these are extracted from `form_data` via `mapSlipForResponse()` for API responses.
...
- Production: `vite build` creates static assets for each app, which are then served by `proxy-server.mjs` in a static file serving mode. Run command sets `NODE_ENV=production`. Detection also checks `REPL_DEPLOYMENT=1`, `REPLIT_DEPLOYMENT=1`, `WEB_REPL_RENEWAL`, or dist files existing without `--dev` flag. Dev mode uses `--dev` flag in start-dev.sh.
